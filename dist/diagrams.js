...
/***/ function(module, exports, __webpack_require__) {
	...
	exports['default'] = function () {
	  ...
	  var addConnection = function addConnection(connection) {
	    if (isArray(connection)) each(connection, addConnection);else if (isNumber(connection)) addConnection({
	      nodesIds: [connection]
	  ...
	  if (args.length > 1) {
	    ...
	    if (isString(connections)) {
	      ...
	      each(connections, function (connection) {
	        if (isNumber(connection)) addDefaultConnectionFromNumber(connection);else addConnection(connection);
	      });
	    } else if (isNumber(connections)) node.id = connections;
	
	    if (args.length > 2) node.description = args[2];
	
	    if (args.length > 3) node.options = (0, _generateNodeOptions2['default'])(args[3]);
	  }
